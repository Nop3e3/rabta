...
const CATEGORIES = [
  {
    id: 'fabrics',
    label: 'Fabrics',
    bg: 'https://images.unsplash.com/photo-1601056639638-c53c50e13ead?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxzZWFyY2h8OHx8ZmFicmljfGVufDB8fDB8fHww',
    fallback: 'linear-gradient(135deg,#3d2b1f,#7a5540)',
  },
  {
    id: 'garments',
    label: 'Garments',
    bg: 'https://images.unsplash.com/photo-1489987707025-afc232f7ea0f?w=240&h=130&fit=crop&auto=format',
    fallback: 'linear-gradient(135deg,#1e1212,#7a2e2e)',
  },
  {
    id: 'packaging',
    label: 'Packaging',
    bg: 'https://images.unsplash.com/photo-1595246135406-803418233494?q=80&w=1170&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    fallback: 'linear-gradient(135deg,#2e2412,#7a6030)',
  },
  {
    id: 'accessories',
    label: 'Accessories',
    bg: 'https://images.unsplash.com/photo-1548036328-c9fa89d128fa?w=240&h=130&fit=crop&auto=format',
    fallback: 'linear-gradient(135deg,#121e12,#3a5a20)',
  },
  {
    id: 'trims',
    label: 'Trims',
    bg: 'https://plus.unsplash.com/premium_photo-1683141087587-433907f9fdfd?q=80&w=1170&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    fallback: 'linear-gradient(135deg,#12121e,#2a2a50)',
  },
];

const MATERIAL_TYPES = [
  'Cotton', 'Polyester', 'Linen', 'Silk', 'Wool',
  'Denim', 'Leather', 'Synthetic Blend', 'Nylon', 'Viscose', 'Other',
];

const COLOR_OPTIONS = [
  'White', 'Black', 'Navy Blue', 'Grey', 'Red', 'Green',
  'Yellow', 'Pink', 'Brown', 'Beige', 'Multicolor', 'Custom / Pantone',
];
...
function CategoryCard({ cat, selected, onSelect }) {
  ...
  return (
    <div
      className={`qf-cat-card ${selected ? 'selected' : ''}`}
    ...
    >
      <div
        className="qf-cat-bg"
        style={{
          background: imgError ? cat.fallback : undefined,
        }}
      >
        {!imgError && (
          <img
...
export default function StepOne({ formData, updateField, errors, onNext }) {
  ...
  const handleImageUpload = (e) => {
    ...
    Promise.all(readers).then(newImgs => {
      updateField('referenceImages', [...current, ...newImgs]);
    });
    ...
    <div className="qf-step-content">
      ...
      <div className="qf-section">
        ...
        <div className="qf-radio-stack">
          <RadioOption
            value="broadcast"
            selected={formData.supplier === 'broadcast'}
            onChange={v => updateField('supplier', v)}
            label="Broadcast to All Suppliers"
            description="Get quotes from multiple verified suppliers (Recommended)"
          />
          <RadioOption
            value="specific"
            selected={formData.supplier === 'specific'}
            onChange={v => updateField('supplier', v)}
            label="Specific Supplier"
            description="Send request to a supplier you've worked with"
          />
          <RadioOption
            value="favorites"
            selected={formData.supplier === 'favorites'}
            onChange={v => updateField('supplier', v)}
            label="Select from Favorites"
            description="Choose from your saved suppliers"
          />
        </div>
      </div>
      ...
      <div className="qf-section">
        <div className="qf-section-header">
          <h2 className="qf-section-title">
            Product Category <span className="qf-req">*</span>
          </h2>
          ...
          <div className="qf-cat-top">
            {CATEGORIES.slice(0, 3).map(cat => (
              <CategoryCard
                key={cat.id}
                cat={cat}
                selected={formData.productCategory === cat.id}
                onSelect={id => updateField('productCategory', id)}
              />
            ))}
          </div>
      ...
      <div className="qf-section">
        <div className="qf-section-header">
          <h2 className="qf-section-title">
            Basic Information <span className="qf-req">*</span>
          </h2>
        </div>

        <div className="qf-field">
          <label className="qf-field-label">
            Product Name <span className="qf-req">*</span>
          </label>
          <input
            className={`qf-input ${errors.productName ? 'has-error' : ''}`}
            type="text"
        ...
        <div className="qf-field">
          <label className="qf-field-label">
            Description <span className="qf-req">*</span>
          </label>
          <textarea
            className={`qf-textarea ${errors.description ? 'has-error' : ''}`}
      ...
      <div className="qf-section">
        <div className="qf-section-header">
          <h2 className="qf-section-title">
            Reference Images <span className="qf-req">*</span>
          </h2>
        ...
        <div>
          ...
          {images.length < MAX_IMAGES && (
            <div
              className={`qf-img-upload ${errors.referenceImages ? 'has-error' : ''}`}
              style={{ marginTop: images.length > 0 ? '10px' : '0' }}
              onClick={() => fileInputRef.current?.click()}
              role="button"
              tabIndex={0}
              onKeyDown={e => e.key === 'Enter' && fileInputRef.current?.click()}
            >
              <span className="qf-img-plus">+</span>
              <span className="qf-img-lbl">Add Image</span>
            </div>
          )}
          ...
          <p className="qf-img-hint" style={{ marginTop: '6px' }}>
      ...
      <div className="qf-section">
        <div className="qf-section-header">
          <h2 className="qf-section-title">
            Material Details <span className="qf-req">*</span>
          </h2>
        </div>

        <div className="qf-field">
          <label className="qf-field-label">
            Material Type <span className="qf-req">*</span>
          </label>
          <select
            className={`qf-select ${errors.materialType ? 'has-error' : ''}`}
        ...
        <div className="qf-field">
          ...
          <textarea
            ...
            onChange={e => updateField('weightThickness', e.target.value)}
            rows={2}
          />
        </div>
      </div>
      ...
      <div className="qf-section">
        <div className="qf-section-header">
          <h2 className="qf-section-title">
            Quantity &amp; Color <span className="qf-req">*</span>
          </h2>
        </div>

        <div className="qf-field">
          <label className="qf-field-label">
            Quantity <span className="qf-req">*</span>
          </label>
          <div className="qf-qty-row">
            <input
              className={`qf-qty-inp ${errors.quantity ? 'has-error' : ''}`}
              type="number"
              min="1"
              placeholder="MOQ ~30"
        ...
        <div
          className={`qf-chk-row ${formData.flexibleQuantity ? 'checked' : ''}`}
          onClick={() => updateField('flexibleQuantity', !formData.flexibleQuantity)}
          role="checkbox"
          aria-checked={formData.flexibleQuantity}
          tabIndex={0}
          onKeyDown={e => e.key === 'Enter' && updateField('flexibleQuantity', !formData.flexibleQuantity)}
        >
          <div className={`qf-chk-box ${formData.flexibleQuantity ? 'checked' : ''}`}>
            {formData.flexibleQuantity && <span className="qf-chk-mark">✓</span>}
          </div>
        ...
        <div className="qf-field">
          ...
          <select
            className={`qf-select ${errors.colorRequirements ? 'has-error' : ''}`}
            value={formData.colorRequirements}
            onChange={e => updateField('colorRequirements', e.target.value)}
          >
